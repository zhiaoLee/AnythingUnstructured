...
from src.config.ocr_content_type import BlockType, ContentType
from src.data.dataset import Dataset, PageableData
from src.libs.boxbase import calculate_overlap_area_in_bbox1_area_ratio, __is_overlaps_y_exceeds_threshold
from src.libs.clean_memory import clean_memory
from src.libs.config_reader import get_local_layoutreader_model_dir, get_llm_aided_config, get_device
from src.libs.convert_utils import dict_to_list
from src.libs.hash_utils import compute_md5
from src.libs.pdf_image_tools import cut_image_to_pil_image
from src.model.magic_model import MagicModel
from src.post_proc.llm_aided import llm_aided_formula, llm_aided_text, llm_aided_title

from src.model.sub_modules.model_init import AtomModelSingleton
from src.post_proc.para_split_v3 import para_split
from src.pre_proc.construct_page_dict import ocr_construct_page_component_v2
from src.pre_proc.cut_image import ocr_cut_image_and_table
from src.pre_proc.ocr_detect_all_bboxes import ocr_prepare_bboxes_for_layout_split_v2
from src.pre_proc.ocr_dict_merge import fill_spans_in_blocks, fix_block_spans_v2, fix_discarded_block
from src.pre_proc.ocr_span_list_modify import get_qa_need_list_v2, remove_overlaps_low_confidence_spans, \
...
def do_predict(boxes: List[List[int]], model) -> List[int]:
    from src.model.sub_modules.reading_oreder.layoutreader.helpers import (
...
def cal_block_index(fix_blocks, sorted_bboxes):
    ...
    if sorted_bboxes is not None:
        ...
        from src.model.sub_modules.reading_oreder.layoutreader.xycut import \
            recursive_xy_cut
...
def pdf_parse_union(
    ...
    with open(os.path.join(os.path.dirname(imageWriter._parent_dir), "pdf_info_dict.json"), "w", encoding="utf-8") as f:
        json.dump(
            pdf_info_dict,
            f,
            ensure_ascii=False,  # 禁止转义中文，保留原始字符
