...
from src.config.make_content_config import DropMode, MakeMode
from src.config.ocr_content_type import BlockType, ContentType
from src.libs.commons import join_path
from src.libs.config_reader import get_latex_delimiter_config
from src.libs.language import detect_lang
from src.libs.markdown_utils import ocr_escape_special_markdown_char
from src.post_proc.para_split_v3 import ListLineTag
                            ...
                            for span in line['spans']:
                                if span['type'] == ContentType.Table:
                                    ...
                                    if span.get('html', ''):
                                        ...
                                        from src.utils.tool import convert_html_to_markdown
                                        html = span['html']
                                        table_md = convert_html_to_markdown(html)
                                        # html_clean = re.sub(r'^<!DOCTYPE html>\s*', '', html)
                                        # html_clean = re.sub(r'^\s+', '', html_clean)
                                        # soup = BeautifulSoup(html_clean, "html.parser")
                                        # body_content = soup.body.decode_contents()
                                        # table_md = md(body_content)
                                        # 添加以下代码降10分
                                        # try:
                                        #     table_md = re.sub(r'^\|(\s*\|\s*)*\n', '', table_md, count=1)
                                        # except:
                                        #     print("删除首行空行错误")
                                        para_text += f"{table_md}"
